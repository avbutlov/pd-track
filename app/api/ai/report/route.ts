...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (startDate && endDate)
      where.deviationDate = { gte: new Date(startDate), lte: new Date(endDate) }

    const total = await prisma.deviation.count({ where })
    const major = await prisma.deviation.count({
      where: { ...where, severity: "major" },
    })
    const minor = await prisma.deviation.count({
      where: { ...where, severity: "minor" },
    })
    ...
    const completedCapas = await prisma.cAPA.count({
      where: { deviation: where, status: "completed" },
    })
...
${bySite
  ...
  .map(
    (s) =>
      `  ${siteMap[s.siteId]?.name} (${siteMap[s.siteId]?.code}): ${s._count.id}`
  )
  .join("\n")}
...
`
    ...
    const period =
      startDate && endDate
        ? `${locale === "ru" ? "С " : ""}${startDate} ${locale === "ru" ? "по" : "to"} ${endDate}`
        : locale === "ru"
          ? "За всё время"
          : "All time"
    ...
    const savedReport = await prisma.report.create({
      data: {
        title: `${locale === "ru" ? "Отчёт для спонсора" : "Sponsor Report"} — ${period}`,
        ...
        statistics: {
          ...
          bySite: bySite.map((s) => ({
            ...s,
            siteName: siteMap[s.siteId]?.name,
            siteCode: siteMap[s.siteId]?.code,
          })),
          capas,
          completedCapas,
        },
